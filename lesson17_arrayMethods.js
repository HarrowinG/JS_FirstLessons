...
//1. get num and return numbers array from 1 to num
...
//3. func take any arrays as input, remove first element of each and return resulted array
function changeCollection() {
  let res = [];
  for (let i = 0; i < arguments.length; i++) {
    if (Array.isArray(arguments[i])) {
      let item = arguments[i].slice();
      item.shift();
      res.push(item);
    }
  }
...
//4. filter users by all values passed, check that parameters passed
const myUsers = [
  {
...
console.log(filterUsers(myUsers, "age", 36));